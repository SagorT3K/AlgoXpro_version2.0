...
class Browser:
    ...
    def get_cookies(self) -> str:
        ...
        Note: httpx may raise CookieConflict if multiple cookies with the
        same name exist. We defensively dedupe by cookie name and keep
        the latest value.
        """
        jar = self._client.cookies

        # Dedupe by name to avoid httpx CookieConflict during iteration.
        values_by_name: dict[str, str] = {}

        # Prefer .items() when possible (fast path, no conflict).
        try:
            for name, value in jar.items():
                values_by_name[name] = value
        except Exception:
            # Fallback: iterate the underlying http.cookiejar.CookieJar
            # directly, which does not raise CookieConflict.
            try:
                raw_jar = jar.jar
                for c in raw_jar:
                    values_by_name[c.name] = c.value
            except Exception:
                # Last-resort fallback: read from internal _cookies dict.
                for c in getattr(jar, "_cookies", {}).values():
                    if hasattr(c, "keys"):
                        for k in c.keys():
                            try:
                                values_by_name[k] = c[k]
                            except Exception:
                                pass
