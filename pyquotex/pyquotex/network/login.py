...
class Login(Browser):
    ...
    async def get_settings(self) -> tuple[Any, dict[str, Any] | None]:
        ...
        # Find the script tag containing "window.settings" (it may not be
        # the first one)
        script_content = "{}"
        ...
        # Update cookies first - they are needed for the WebSocket auth
        self.cookies = self.get_cookies()
        self.api.session_data["cookies"] = self.cookies or ""
        ...
        # Extract token from the HTTP cookies. The session token is stored
        # in an HttpOnly cookie by the Quotex server after login.
        token = None
        try:
            raw_jar = jar.jar
            for cookie in raw_jar:
                # Look for a long token-like value (not a simple word like "pt" or URL)
                # Cookies like laravel_session and remember_web contain the session
                if cookie.name in ("laravel_session",) or cookie.name.startswith("remember_web"):
                    val = cookie.value
                    if len(val) > 20:  # session tokens are long
                        token = val
                        # Prefer HttpOnly cookies (they contain the real token)
    ...
    async def success_login(self) -> tuple[bool, str]:
        ...
        if "trade" in response_url:
            ...
            # Fetch the real WS token from the API digest endpoint.
            # The cookie-based token from get_settings() is the encrypted
            # Laravel session payload, NOT the WebSocket session token.
